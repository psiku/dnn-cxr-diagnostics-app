# ==============================================================================
# This file is taken from the repository: https://github.com/ngaggion/CheXmask-Database/blob/main/HybridGNet/models/modelUtils.py and https://github.com/ngaggion/CheXmask-Database/blob/main/HybridGNet/models/HybridGNet2IGSC.py
# Author: Nicolás Gaggion
# License: GNU GPL v3.0 (https://www.gnu.org/licenses/gpl-3.0.html)
#
# Used in the engineering thesis with the permission of the open-source license.
# ==============================================================================

from torch_geometric.nn.conv import MessagePassing
from torch_geometric.nn.conv.cheb_conv import ChebConv as PyGChebConv
from torch_geometric.nn.inits import zeros, normal

# Use the upstream ChebConv implementation directly.
ChebConv = PyGChebConv
...
class EncoderConv(nn.Module):
    def __init__(self, latents=64, hw=32):
        ...
        self.fc_mu = nn.Linear(
            in_features=self.size[4] * hw * hw, out_features=self.latents
        )
...
class Hybrid(nn.Module):
    def __init__(
        self, config, downsample_matrices, upsample_matrices, adjacency_matrices
    ):
        ...
        self.graphConv_up4 = ChebConv(
            self.filters[4] + outsize1 + 2, self.filters[3], self.K
        )
        self.graphConv_up3 = ChebConv(self.filters[3], self.filters[2], self.K)

        self.SC_2 = SkipBlock(self.filters[2], self.window)

        self.graphConv_up2 = ChebConv(
            self.filters[2] + outsize2 + 2, self.filters[1], self.K
        )
    ...
    def _graph_conv(self, layer, x, edge_index):
    ...
    def forward(self, x):
        ...
        x = self._graph_conv(
            self.graphConv_up6, x, self.adjacency_matrices[5]._indices()
        )
        x = self.normalization6u(x)
        x = F.relu(x)

        x = self._graph_conv(
            self.graphConv_up5, x, self.adjacency_matrices[4]._indices()
        )
        ...
        x = self._graph_conv(
            self.graphConv_up4, x, self.adjacency_matrices[3]._indices()
        )
        ...
        x = self._graph_conv(
            self.graphConv_up3, x, self.adjacency_matrices[2]._indices()
        )
        ...
        x = self._graph_conv(
            self.graphConv_up2, x, self.adjacency_matrices[1]._indices()
        )
        x = self.normalization2u(x)
        x = F.relu(x)

        x = self._graph_conv(
            self.graphConv_up1, x, self.adjacency_matrices[0]._indices()
        )  # Sin relu y sin bias
