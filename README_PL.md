...
3. Pobranie plików z wagami modeli modeli:
   - Model segmentacyjny należy pobrać stąd: https://drive.google.com/file/d/1yNCaLjKooiDQ2dDEfdK2K88s9_E8hpJS/view?usp=sharing. Następnie wagi modelu ***segmentation_model_bestMSE.pt*** należy przerzucić do folderu ***backend/models/***
   - Wagi modeli wytrenowanych należy pobrać stąd:
...
Przed pierwszym uruchomieniem należy zweryfikować zawartość pliku `backend/config/model_config.yml`. Ścieżki do plików modeli muszą być podane względem katalogu `backend`, ponieważ tylko wtedy są poprawnie odczytywane zarówno przez serwer uruchamiany lokalnie, jak i wewnątrz kontenera. Wymagane wartości to `models/app/last.ckpt` dla pola `model_path`, `config/best_thresholds.json` dla pola `thresholds_path` oraz `models/segmentation_model_bestMSE.pt` dla pola `segmentation_weights_path`. Liczba klas zadeklarowana w polu `num_classes` musi być zgodna z liczbą patologii opisanych w pliku `backend/config/pathologies.json` oraz z liczbą progów decyzyjnych; niezgodność jest wykrywana podczas uruchamiania serwera i sygnalizowana komunikatem błędu.
